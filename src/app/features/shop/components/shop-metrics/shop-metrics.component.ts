...
import { ShopDashboardStats, ShopStatsService } from '../../../../services/services/shop-stats.service';

@Component({
  selector: 'app-shop-metrics',
  standalone: true,
  imports: [
    BadgeComponent,
    SafeHtmlPipe,
  ...
  templateUrl: './shop-metrics.component.html',
  styleUrl: './shop-metrics.component.css',
})
export class ShopMetricsComponent implements OnInit {
  ...
  totalOrders = 0;
  customerVariation = 0;
  customerTrend: 'up' | 'down' = 'up';
  orderVariation = 0;
  orderTrend: 'up' | 'down' = 'down';
  isLoading = true;
  hasError = false;
  ...
  ngOnInit(): void {
  this.shopStatsService.getShopDashboardStats().subscribe({
    next: (response: any) => {
      const data = response.data; // ← unwrap { success, data }

      this.totalCustomers = data.metrics.customers.total;
      this.customerVariation = data.metrics.customers.variation;
      this.customerTrend = data.metrics.customers.trend;

      this.totalOrders = data.metrics.orders.total;
      this.orderVariation = data.metrics.orders.variation;
      this.orderTrend = data.metrics.orders.trend;

      this.series = [...data.monthlySales.series];
      this.xaxis = { ...this.xaxis, categories: [...data.monthlySales.categories] };
